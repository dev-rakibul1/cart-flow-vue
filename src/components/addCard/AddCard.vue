  ...
  >
    ...
    <div class="flex-grow px-3 w-full">
      ...
      <div class="flex mt-1 space-x-2 justify-start items-center">
        ...
        <span>|</span>
        <span class="text-base font-normal">Qty: {{ product.quantity }}</span>
      </div>
    </div>
    ...
    <div class="flex items-center space-x-1.5">
      ...
      <span class="w-8 text-center font-medium text-gray-900 select-none">
        {{ quantity }}
      </span>

      <button
        @click="handleAddToCard(product)"
        class="w-7 h-7 flex items-center justify-center rounded-full bg-gray-100 hover:bg-gray-200 active:bg-gray-300 text-gray-600 transition-colors duration-200"
      >
        <span class="transform translate-y-[-1px]">+</span>
      </button>
    </div>
    ...
    <button
      ...
      class="ml-3 p-2 text-gray-400 hover:text-red-500 transition-colors duration-200 rounded-full hover:bg-red-50"
      title="Remove item"
    >
      <svg
        class="w-4 h-4"
