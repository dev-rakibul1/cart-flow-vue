...
import { computed, onMounted } from "vue"; // Import computed and onMounted
export default {
  name: "ProductCart",
  props: {
    drink: {
  ...
  <div class="w-full bg-white rounded-lg border">
    ...
    <div class="rounded-t-lg overflow-hidden">
      <img
        :src="
          drink?.image ||
          'https://eda.yandex/images/12773219/e9697d4bfec52220ff085dcab7cf1f3e-216x188.jpeg'
        "
        alt="Food Combo"
    ...
    <div class="p-2">
      <!-- Price Section -->
      <div
        class="mt-4 text-lg font-bold text-gray-900 flex items-center justify-between"
      >
        <h1>
          {{ drink?.price ? drink.price + "$" : "Product Price: N/A" }}
        </h1>
        <span class="font-normal flex items-center justify-start rating-start">
          {{ drink?.rating ? drink.rating : "0" }}
          <svg
            width="16px"
            height="16px"
            viewBox="0 0 24 24"
      ...
      <span
        ...
        "
      >
        {{ drink?.isStock ? "In Stock" : "Out of Stock" }}
      </span>

      <!-- Button Section -->
      <div class="mt-4 flex items-center justify-between space-x-3">
        <button
          @click="handleAddToCard(drink)"
          :disabled="!drink?.isStock"
          :class="[
